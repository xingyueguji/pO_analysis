...
if grep -q "nPDFerrSet " powheg.input; then
    cp -p ${WORKDIR}/EPPS21NLOR_* .
fi
...
if grep -q "nPDFerrSet " powheg.input.noweight; then
    echo -e "\ncomputing weights for 106+1 EPPS21 nPDF variations\n"
    for iset in {1..107}; do
        echo -e "EPPS21 set ${iset}\n"
        sed -e 's/.*nPDFerrSet.*/nPDFerrSet '${iset}'/' powheg.input.noweight > powheg.input
        sed -i '/rwl_file/d' powheg.input
        echo "rwl_file '-'" >> powheg.input
        echo "rwl_add 1" >> powheg.input
        echo "<initrwgt>" >> powheg.input
        echo "<weightgroup name='EPPS21_variation' combine='hessian' >" >> powheg.input
        echo "<weight id='$((9000+${iset}))'> nPDFerrSet=${iset} </weight>" >> powheg.input
