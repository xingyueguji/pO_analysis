...
C "EPPS21: Nuclear parton distributions with LHC data."
C Published in EPJC
C Eprint: arXiv:2112.12462
C
...
C                      compatibility with the EPS09/EPPS16 routine
C
C
C     pset (integer):
C            1       = central fit
...
C            ...   ...
C            106,107 = error sets {S-53}, {S+53}
C
...
C the command
C
C   Call EPPS21(order, pset, A, x, Q, ruv, rdv, ru, rd, rs, rc, rb, rg)
C
...
C in Secs. 4.2 and 4.3 of arXiv:2112.12462.
C
C *********************************************************
C *********************************************************/
      ...
      Subroutine EPPS21(order, pset, AAA, xxx, QQQ,
     &                   ruv, rdv, ru, rd, rs, rc, rb, rg)

      Implicit none
      Double precision :: ruv, rdv, ru, rd, rs, rc, rb, rg, QQQ, xxx
      Double precision :: LSTEP, x, Q, Q2
      Double precision :: allvalues(1:107,1:8,0:50,0:250)
      ...
      Double precision :: n_x

      Character (Len=50) filenimi

      Integer :: Qsteps=30, xsteps=250
...
C *********************************************
      ...
      If (pset .LT. 1 .or. pset .GT. 107) then
      Write(*,*) 'In EPPS21: Wrong set!', pset
      Write(*,*) 'Central set: pset = 1'
      Write(*,*) 'Error sets : pset = 2...107'
      Stop
      End If
...
C ************************************
      ...
      If (Q2 .LT. Q2min) Then
      Write(*,*) "In EPPS21: Q<1.3GeV"
      stop
      End If
      If (Q2 .GT. Q2max) Then
      Write(*,*) "In EPPS21: Q>10000GeV"
      stop
      End If
...
C Read the table
        ...
        If (A < 10) Then
        Write(filenimi,'("EPPS21NLOR_", I1)'), A
        Else If (A < 100) Then
        Write(filenimi,'("EPPS21NLOR_", I2)'), A
        Else If (A < 1000) Then
        Write(filenimi,'("EPPS21NLOR_", I3)'), A
        End If

      Call NextUnitEPPS21(openchannel)

      OPEN (openchannel, file = filenimi, status='OLD', IOSTAT=iovar)

      If (iovar .NE. 0) Then
      Write(*,*) 'In EPPS21: Missing file: ',filenimi
      stop
      End If
...
C Find the position in the x-grid
      ...
      LSTEP = (0.d0 - (Log(1.d0/x_i) + 5.d0*(1-x_i)))/(1.d0*xsteps)

      n_x  = ((Log(1.d0/x)+5.d0*(1-x))-(Log(1/x_i)+5.d0*(1-x_i)))/LSTEP
      xpoint = Aint(n_x)
...
c For bottom, only use points Q>4.75 for interpolation
      ...
      Call luoviEPPS21(fu,arg,4,n_x,res)
      fg(j) = res
...
C *****************************************
      ...
      Call luoviEPPS21(fg,arg,4,realQ,res)
...
! ********************************
      ...
      SUBROUTINE luoviEPPS21(F,ARG,MMM,Z,SUM)
 ...
 1810 CONTINUE
      ...
      End SUBROUTINE luoviEPPS21

! **********************
! Find the open I/O unit
! **********************

      Subroutine NextUnitEPPS21(firstopen)
