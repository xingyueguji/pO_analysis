...
if [[ $${period} == *"_EPPS21_"* ]]; then
    patch -l -p0 -i ${patches_dir}/EPPS21/lhapdfif_nPDF.patch
    patch -l -p0 -i ${patches_dir}/EPPS21/lhapdf6if_nPDF.patch
    patch -l -p0 -i ${patches_dir}/EPPS21/pdfcalls_nPDF.patch
    cp ${patches_dir}/EPPS21/EPPS21.f POWHEG-BOX/$${process}/
    if [[ $${period} == *"_O" ]]; then
        wget --no-verbose --no-check-certificate -P POWHEG-BOX/$${process}/$${subprocess} http://anstahll.web.cern.ch/anstahll/powheg/EPPS21NLOR_16
    elif [[ $${period} == *"_Ne" ]]; then
        wget --no-verbose --no-check-certificate -P POWHEG-BOX/$${process}/$${subprocess} http://anstahll.web.cern.ch/anstahll/powheg/EPPS21NLOR_20
    elif [[ $${period} == *"_Xe" ]]; then
        wget --no-verbose --no-check-certificate -P POWHEG-BOX/$${process}/$${subprocess} http://anstahll.web.cern.ch/anstahll/powheg/EPPS21NLOR_131
    elif [[ $${period} == *"_Pb" ]]; then
        wget --no-verbose --no-check-certificate -P POWHEG-BOX/$${process}/$${subprocess} http://anstahll.web.cern.ch/anstahll/powheg/EPPS21NLOR_208
    fi
fi
...
if [[ $${period} == *"_EPPS21_"* ]]; then
    echo "PWHGANAL=$${BOOK_HISTO} pwhg_analysis-dummy.o EPPS21.o" >> tmpfile
fi
