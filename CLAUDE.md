...
- **SF APPLICATION PHASE STARTED 2026-09-09 — step 1 DONE: the trigger
  turn-on from data vs W signal MC with the minimum-bias denominator**
  (`correction/trig_eff_mb.C` + `run_trig_eff_mb.sh`, see the `correction/`
  bullet): MB path verified in data (run-dependent prescale, none in runs
  393974/393975) and MC (99.9%); pT > 25 SF μ 0.996 ± 0.002, e 0.989 ± 0.003
  plain / 0.997 ± 0.004 with m_T > 40 (the difference = the fake composition
  of the data sample). NEXT: tag-and-probe on the Z samples for the per-lepton
  ID/iso/trigger SFs (no T&P code yet — the AN-outline gap), then fold the SFs
  into the skim weight `w` (propagates to r automatically, σ = r × σ_gen).
- **LHE WEIGHTS → nPDF SYSTEMATIC, SKIM STAGE DONE 2026-09-07** — every MC
  ...
  **σ = r×σ_gen(nominal): W⁺ 55.24 ± 3.26 / W⁻ 44.63 ± 2.63 / W 99.87 ± 5.75
  ...
  \pPb, a `\newlabel` stub for `eq:infit` and the HIN-17-007 bib entry). **NEXT (decided by this result): (1) the
