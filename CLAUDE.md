...
- `test/my_script/extract_pO_simfit.C` — **2026-09-07: 9th arg `lheSysts`
...
- `test/run_pO_impacts.sh` (2026-08-17) — nuisance impacts + covariance plots
  ...
  `simfit/cov/`. `sync_lxplus.sh download` pulls both dirs (excluding `wd_*`);
...
- **SF APPLICATION PHASE — step 2 DONE 2026-09-14: MUON SFs FOLDED INTO THE
  ...
  **ONE combined nuisance `muSF`** (user decision: the three independent
  ...
  = the data sample's fake composition). **NEXT: refit on lxplus**
  (`sync_lxplus.sh upload`, `QCD_MODE=abcd ./run_pO_fits.sh --disc leppt_mt40
  --asimov`, download, `run_observables.sh leppt_mt40` + `postfit_incl`) —
  expect the μ-channel MC down ~1.2% with r up accordingly and the 12
  `muTrig_y*` pulls ≈ 0 ± 1; then the electron SFs (same pattern;
...
- **IN-FIT ABCD QCD (2026-08-23, `QCD_MODE=abcd`)** — the leppt_mt40 QCD
  ...
  **0.654±0.053 with θ_e⁻ = −2.6σ** (fb variant same pattern:
  ...
  turn-on TnP SFs, not QCD; (iii) the +/− split = charge composition: data
