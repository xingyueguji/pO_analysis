// correction/recoil_raw.C
//
// Raw look at the Z->mumu hadronic-recoil components (u_par, u_perp) used for
...
//
// Inputs : ../skim/rootfile/ZToMuMu_pO2025_Data_hist.root   (data)
//          ../skim/rootfile/ZToMuMu_pO2025_DY_MC_hist.root  (DY signal MC)
//          histos: h_uPar, h_uPerp (1D inclusive); h_uPar_qT, h_uPerp_qT (2D vs q_T)
// Output : ./plots/recoil_Zmm/  (i.e. correction/plots/recoil_Zmm/)
...
//
// Run from correction/:  root -l -q 'recoil_raw.C+'
...
// refine once you see the entry table. Edit freely -- these are projections.
...
void recoil_raw()
{
    ...
    TFile *fData = TFile::Open((dir + "ZToMuMu_pO2025_Data_hist.root").c_str(), "READ");
    if (!fData || fData->IsZombie())
    {
        std::cerr << "[ERR] cannot open " << dir << "ZToMuMu_pO2025_Data_hist.root\n";
        return;
    }
    TFile *fMC = TFile::Open((dir + "ZToMuMu_pO2025_DY_MC_hist.root").c_str(), "READ");
    if (!fMC || fMC->IsZombie())
    {
        std::cerr << "[ERR] cannot open " << dir << "ZToMuMu_pO2025_DY_MC_hist.root\n";
        return;
    }
                                           ...
                                           {"h_uPerp", "u_{#perp} [GeV]"}})
    {
        ...
        SaveDataMCRatio(hD, hM, outDir + "/" + o.name,
                        o.xt, "Events (a.u.)",
                        "Z#rightarrow#mu#mu recoil (inclusive)",
        ...
        for (size_t i = 0; i + 1 < kQtEdges.size(); ++i)
        {
            ...
            if (pd->Integral() > 0 || pm->Integral() > 0)
                SaveDataMCRatio(pd, pm,
                                ...
                                c.xt, "Events (a.u.)",
                                Form("Z#rightarrow#mu#mu: q_{T} #in [%.0f, %.0f) GeV", qlo, qhi),
                                "Data vs DY MC", "shape-normalized",
                                ps, true, "Data", "DY MC");
            delete pd;
            delete pm;
